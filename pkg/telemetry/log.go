...
import (
	...
	"github.com/tolgaOzen/go-skeleton/pkg/telemetry/logexporters"
)
...
func NewGCPHandler(headers map[string]string, level slog.Leveler) (slog.Handler, error) {
	...
	gcpHandler := sloggcp.NewGoogleCloudSlogHandler(context.Background(), projectId, "skeleton", &slog.HandlerOptions{
		Level: level,
	})
