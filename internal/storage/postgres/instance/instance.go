...
func PostgresDB(postgresVersion string) database.Database {
	...
	postgres, err := testcontainers.GenericContainer(ctx, testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			...
			Env:          map[string]string{"POSTGRES_USER": "postgres", "POSTGRES_PASSWORD": "postgres", "POSTGRES_DB": "skeleton"},
	...
	cmd := []string{"sh", "-c", "export PGPASSWORD=postgres" + "; psql -U postgres -d skeleton -c 'DROP SCHEMA public CASCADE; CREATE SCHEMA public;'"}
	...
	postgresDSN := fmt.Sprintf("postgres://%s:%s@%s/%s?sslmode=disable", "postgres", "postgres", dbAddr, "skeleton")
